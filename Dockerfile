...
RUN TAILWIND_CSS_VERSION="$(uv run python3 -c 'from capella_model_explorer.__main__ import DEFAULTS as d; print(d["TAILWIND_CSS_VERSION"])')" && \
  TAILWIND_CSS_CLI_VERSION="$(uv run python3 -c 'from capella_model_explorer.__main__ import DEFAULTS as d; print(d["TAILWIND_CSS_CLI_VERSION"])')" && \
  TAILWIND_CSS_TYPOGRAPHY_VERSION="$(uv run python3 -c 'from capella_model_explorer.__main__ import DEFAULTS as d; print(d["TAILWIND_CSS_TYPOGRAPHY_VERSION"])')" && \
  export TAILWIND_CSS_VERSION && \
  export TAILWIND_CSS_CLI_VERSION && \
  export TAILWIND_CSS_TYPOGRAPHY_VERSION
RUN npm install -D \
