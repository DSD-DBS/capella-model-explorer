...
export const DiffExplorer = ({
  ...
  const getIcon = (changed, attributes) => {
    ...
    switch (changed) {
      ...
      case 'modified':
        return hasNonEmptyAttributes ? (
          <SquareAsterisk className="h-5 w-5 text-orange-500" />
        ) : (
  ...
  const renderNode = (node) => {
    ...
    return (
      <div
        ...
        <div className={`flex items-center ${hasChildren ? '' : ''}`}>
          {hasChildren && (
            <span
              onClick={() => handleToggle(node.uuid)}
              className="mr-2 cursor-pointer">
              {isExpanded ? (
                <SquareChevronDown className="dark:text-gray-200" />
              ) : (
                <SquareChevronRightIcon className="dark:text-gray-200" />
              )}
            </span>
          )}
          <span
            className={`mr-3 flex h-6 w-6 items-center justify-center rounded-full ${getNodeClass(node.change, node.attributes)}`}>
