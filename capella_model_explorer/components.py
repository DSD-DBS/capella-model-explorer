...
def navbar(template: reports.Template | None, element: str | None) -> ft.Nav:
    ...
    return fc.Mdui_top_app_bar(
        fc.Mdui_button_icon(
            icon="menu",
            onclick="template_sidebar.open = !template_sidebar.open",
        ),
        title,
        ft.Div(cls="grow"),
        ft.Div(
...
def template_card(template: reports.Template) -> ft.A:
    ...
    if reports.TemplateFlags.EXPERIMENTAL in template.flags:
        c = fc.Mdui_chip(
            "Experimental", icon="science--rounded", cls="chip-experimental"
        )
        chips.append(c)

    if reports.TemplateFlags.STABLE in template.flags:
        c = fc.Mdui_chip("Stable", icon="gpp_good--rounded", cls="chip-stable")
        chips.append(c)
...
def template_sidebar(
    *,
    ...
    return fc.Mdui_navigation_drawer(
        sidebar_caption,
